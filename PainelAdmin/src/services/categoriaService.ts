...
export const categoriaService = {
  ...
  listar: async (): Promise<Category[]> => {
    try {
      ...
      const response = await api.get('/CategoriaApi');

      console.log('📥 RESPOSTA BRUTA - CATEGORIAS:');
      ...
      const categorias: Category[] = response.data.map(
        (c: any): Category => ({
          ...
          isActive: c.ativo ?? true,
          tourCount: c.tourCount ?? 0,

          // NOVO: Mapeamento dos novos campos
          classificacao: c.classificacao ?? 'TEMA',
          cidadesVinculadas: c.cidadesVinculadas ?? [],

          audit: c.atualizado_em
            ? {
                lastEditedBy: c.adminAtualizacao?.nome_completo ?? 'Admin',
                lastEditedAt: new Date(c.atualizado_em).toLocaleString('pt-BR'),
              }
            : undefined,
        })
      );

      console.log('🔄 CATEGORIAS APÓS CONVERSÃO:');
      console.log(categorias);
      console.log(`✅ TOTAL DE CATEGORIAS RECEBIDAS: ${categorias.length}`);
  ...
  uploadImagem: async (imagem: File): Promise<string> => {
    ...
    try {
      ...
      const response = await api.post('/CategoriaApi/upload-imagem', formData);

      console.log('✅ IMAGEM DA CATEGORIA ENVIADA:');
      console.log(response.data);
  ...
  criar: async (categoria: Partial<Category>, photoFile?: File | null) => {
    ...
    const payload = {
      ...
      ImgUrl: imagemUrl,
      Ativo: categoria.isActive ?? true,
      Classificacao: categoria.classificacao ?? 'TEMA',
      CidadesVinculadas: categoria.cidadesVinculadas ?? [],
    };
